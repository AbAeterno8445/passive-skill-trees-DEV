...
function PST:onRoomClear(RNG)
    ...
    local roomIdxStr = tostring(level:GetCurrentRoomIndex())
    local clearsPerRoom = PST:getTreeSnapshotMod("clearsPerRoom", {})
    if not clearsPerRoom[roomIdxStr] then clearsPerRoom[roomIdxStr] = 0 end
    clearsPerRoom[roomIdxStr] = clearsPerRoom[roomIdxStr] + 1
