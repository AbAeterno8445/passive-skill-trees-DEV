    ...
    PST:getRoom():GetFrameCount() > 1 then
        ...
        if PST:isValidMobDeath(entity) and (not isSegmentBoss or (isSegmentBoss and PST:isLastMobOfType(entity))) then
            ...
            if entity.Type == EntityType.ENTITY_DELIRIUM then
                PST:addModifiers({ roomDeliriumKill = true }, true)
            end
            ...
            if addXP and not PST:getTreeSnapshotMod("d7Proc", false) and not PST:getTreeSnapshotMod("deathTrialActive", false) then
                ...
                if entity:GetBurnCountdown() > 0 then
                    ...
                    -- Astral Companion: Hell Hound effect
                    local tmpMod = PST:getTreeSnapshotMod("hHoundBurnKillDmg", 0)
                    if tmpMod > 0 and PST:getTreeSnapshotMod("hHoundBurnKillProcs", 0) < 16 then
                        PST:addModifiers({ damagePerc = tmpMod, hHoundBurnKillProcs = 1 }, true)
                    end
                    -- Hell Hound scavenge event and objective
                    PST:astralCompAddProgress("hellHound", 1)
                    PST:astralCompProcScavenge("hellHound")
                    if entity:IsBoss() and PST:getLevel():GetStage() >= 7 then
                        PST:astralCompEggUnlockProc("hellHound")
                    end
                end
                ...
                if entity:GetPoisonCountdown() > 0 then
                    ...
                    if tmpMod > 0 and PST:getTreeSnapshotMod("blackMambaPoisonLuckTotal", 0) < 3 then
                    ...
                    if tmpMod > 0 and PST:getTreeSnapshotMod("waterMocPoisonKillProcs", 0) < 20 then
                        PST:addModifiers({ damagePerc = tmpMod, waterMocPoisonKillProcs = 1 }, true)
                    end
                    -- Water Moccasin scavenge event
                    if entity.MaxHitPoints >= 40 then
                        PST:astralCompProcScavenge("waterMoccasin")
                    end
                end

                -- Petrified enemy death
                if entity:GetFreezeCountdown() > 0 then
                    -- Astral Companion: Anaconda
                    tmpMod = PST:getTreeSnapshotMod("anacondaPetrifKillDmg", 0)
                    if tmpMod > 0 and PST:getTreeSnapshotMod("anacondaPetrifKillProcs", 0) < 16 then
                        PST:addModifiers({ damagePerc = tmpMod, anacondaPetrifKillProcs = 1 }, true)
                    end
                    -- Anaconda scavenge event
                    PST:astralCompProcScavenge("anaconda")
                end

                -- Bleeding enemy death
                if entity:GetBleedingCountdown() > 0 then
                    -- Astral Companion: Culicivora
                    tmpMod = PST:getTreeSnapshotMod("culicivoraBleedKillDmg", 0)
                    if tmpMod > 0 and PST:getTreeSnapshotMod("culicivoraKillProcs", 0) < 16 then
                        PST:addModifiers({ damagePerc = tmpMod, culicivoraKillProcs = 1 }, true)
                    end
                    -- Culicivora scavenge event and objective
                    PST:astralCompAddProgress("culicivora", 1)
                    PST:astralCompProcScavenge("culicivora")
                end
                ...
                if entity:GetFearCountdown() > 0 then
                    ...
                    if tmpMod > 0 and PST:getTreeSnapshotMod("abTarKillProcs", 0) < 16 then
                        PST:addModifiers({ damagePerc = tmpMod, abTarKillProcs = 1 }, true)
                    end
                    -- Abyssal Tarantula scavenge event
                    PST:astralCompProcScavenge("abyssalTarantula")
                end
                ...
                if entity:GetSlowingCountdown() > 0 then
                    ...
                    if tmpMod > 0 and PST:getTreeSnapshotMod("jumpSpiderKillProcs", 0) < 16 then
                        PST:addModifiers({ damagePerc = tmpMod, jumpSpiderKillProcs = 1 }, true)
                    end
                    -- Jumping Spider scavenge event
                    if (NPCisBoss or NPCisChamp) then
                        PST:astralCompProcScavenge("jumpingSpider")
                    end
                end

                -- Any status death
                if PST:entityHasAnyStatus(entity) then
                    -- Astral Companion: Manticore objective
                ...
                if tmpNPC and tmpNPC.MaxHitPoints <= 6 then
                    tmpMod = PST:getTreeSnapshotMod("scorpionHeartOnKill", 0)
                    if tmpMod > 0 and PST:getTreeSnapshotMod("scorpionKillProcs", 0) < 3 and 100 * math.random() < tmpMod then
                    ...
                    -- Scorpion scavenge event and objective
                    PST:astralCompAddProgress("scorpion", 1)
                    PST:astralCompProcScavenge("scorpion")
                end
                -- Astral Companion: Emperor Scorpion
                if tmpNPC and tmpNPC.MaxHitPoints >= 10 and tmpNPC.MaxHitPoints <= 30 then
                    tmpMod = PST:getTreeSnapshotMod("empScorpionSoulOnKill", 0)
                    if tmpMod > 0 and PST:getTreeSnapshotMod("empScorpionProcs", 0) < 2 and 100 * math.random() < tmpMod then
                    ...
                    -- Emperor Scorpion scavenge event and objective
                    PST:astralCompAddProgress("emperorScorpion", 1)
                    PST:astralCompProcScavenge("emperorScorpion")
                end
                -- Astral Companion: Skyshark
                if entity:GetBleedingCountdown() > 0 and entity:IsFlying() then
                    ...
                    -- Skyshark scavenge event and objective
                    PST:astralCompAddProgress("skyshark", 1)
                    PST:astralCompProcScavenge("skyshark")
                    for _, tmpEnem in ipairs(PST.taintedMobs) do
                        if entity.Type == tmpEnem[1] and entity.Variant == tmpEnem[2] then
                            PST:astralCompEggUnlockProc("skyshark")
                            break
                        end
                    end
                end
                ...
                if entity.Type == EntityType.ENTITY_URIEL or entity.Type == EntityType.ENTITY_GABRIEL then
                    PST:astralCompAddProgress("pearlDragon", 1)
                    PST:astralCompProcScavenge("pearlDragon")
                end
                -- Astral Companion: Clockroach scavenge event and objective
                if entity:GetSlowingCountdown() > 0 or isFrozen then
                    PST:astralCompAddProgress("clockroach", 1)
                    PST:astralCompProcScavenge("clockroach")
                end
                -- Astral Companion: Ice Beast scavenge event and objective
                if isFrozen then
                    PST:astralCompAddProgress("iceBeast", 1)
                    if entity.MaxHitPoints >= 20 then
                        PST:astralCompProcScavenge("iceBeast")
                        PST:astralCompEggUnlockProc("iceBeast")
                    end
                end
            ...
            if isFinalBoss then
                ...
                if PST:getTreeSnapshotMod("finalBossKillProcs", 0) < 2 then
                    ...
                    -- Astral Companion: Golden Dragon scavenge event and objective
                    PST:astralCompAddProgress("goldenDragon", 1)
                    PST:astralCompEggUnlockProc("goldenDragon")
                    if PST:getTreeSnapshotMod("finalBossKillProcs", 0) == 1 then
                        PST:astralCompProcScavenge("goldenDragon")
                    end
                end
            ...
            if tmpNPC then
                ...
                if NPCisChamp then
                    ...
                    -- Mod: % chance for champions to drop a random pill on death, once per floor (Bluecap companion)
                    tmpMod = PST:getTreeSnapshotMod("bluecapChampDeathPill", 0)
                    if tmpMod > 0 and not PST:getTreeSnapshotMod("bluecapChampPillProc", false) then
                        Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_PILL, -1, entity.Position, RandomVector() * 3, nil)
                        PST:addModifiers({ bluecapChampPillProc = true }, true)
                    end

                    local lvlStage = PST:getLevel():GetStage()
                    -- Astral Companion: Clockroach egg
                    if entity:GetSlowingCountdown() > 0 then
                    ...
                    if entity:GetFreezeCountdown() > 0 then
                        PST:astralCompEggUnlockProc("anaconda")
                    end
                    -- Astral Companion: Culicivora egg
                    if entity:GetBleedingCountdown() > 0 and lvlStage > 2 then
                        PST:astralCompEggUnlockProc("culicivora")
                    end
                    -- Astral Companion: Abyssal Tarantula egg
                ...
                if NPCisBoss then
                    ...
                    if PST:isRunSidereal() then
                        ...
                        local lvlStage = PST:getLevel():GetStage()
                        local lvlStageType = PST:getLevel():GetStageType()
                        if (lvlStage <= 2 and lvlStageType >= 4) or
                        ((lvlStage == 3 or lvlStage == 4) and lvlStageType == StageType.STAGETYPE_AFTERBIRTH) then
                            PST:astralCompEggUnlockProc("riverRat")
                        end

                        -- Astral Companion: Water Moccasin egg
                        if ((lvlStage == 3 or lvlStage == 4) and lvlStageType == StageType.STAGETYPE_AFTERBIRTH) then
                            PST:astralCompEggUnlockProc("waterMoccasin")
                        end

                        -- Astral Companion: Snapping Turtle scavenge event and objective
                        if room:GetType() ~= RoomType.ROOM_BOSS then
                            PST:astralCompAddProgress("snappingTurtle", 1)
                            PST:astralCompProcScavenge("snappingTurtle")
                            if lvlStage >= 7 then
                                PST:astralCompEggUnlockProc("snappingTurtle")
                            end
                        end
                        ...
                        if PST:arrHasValue(PST.deadlySinBosses, entity.Type) then
                            -- Normal Deadly Sin boss
                            if entity.Variant == 0 then
                                -- Astral Companion: Scorpion egg
                                PST:astralCompEggUnlockProc("scorpion")
                            -- Super Deadly Sin boss
                            elseif entity.Variant == 1 then
                                -- Astral Companion: Emperor Scorpion egg
                                PST:astralCompEggUnlockProc("emperorScorpion")
                            end
                        end
                ...
                -- Mod: % chance for champion and boss monsters to drop a vanishing micro battery on death if you have Old Capacitor (Raiju companion)
                tmpMod = PST:getTreeSnapshotMod("raijuCapacitor", 0)
                if tmpMod > 0 and (NPCisBoss or NPCisChamp) and PST:getPlayer():HasTrinket(TrinketType.TRINKET_OLD_CAPACITOR) and
                100 * math.random() < tmpMod then
                    local newBattery = Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_LIL_BATTERY, BatterySubType.BATTERY_MICRO, entity.Position, RandomVector() * 3, nil)
                    newBattery:ToPickup().Timeout = 210
                end
                ...
                -- Undead 'death'
                if PST:isMobUndead(tmpNPC) then
                    -- Astral Companion: Pharaoh Ant scavenge event and objective
                    PST:astralCompAddProgress("pharaohAnt", 1)
                    PST:astralCompProcScavenge("pharaohAnt")
                    if tmpNPC:IsBoss() then
                        PST:astralCompEggUnlockProc("pharaohAnt")
                    end
                end
            end
            ...
            if PST:SC_getSnapshotMod("nullstone", false) then
                ...
                not NPCisBoss and not PST:arrHasValue(PST.nullstoneBlacklist, tmpNPC.Type) and room:GetType() ~= RoomType.ROOM_BOSS and
